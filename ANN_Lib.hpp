...
double sigm(double sorce) {
    return 1 / (1 +  exp(-(sorce)));
}
...
public:
    FeedforwardANN(std::vector<int> layers_struct, double learning_rate = 0.01, int thread_count = 8):delta(0,1), re(time(0)), sf(learning_rate) {
        ...
        std::uniform_real_distribution<double> initial_rand(0, 0.01);
...
template<class I, class O>
void StacGradDesend(FeedforwardANN& source,I input, O output, double expected_acc, int size_for_fitness = 100, bool is_rand = true) {
    FeedforwardANN current(source);
    if (is_rand)
        current.randomize();
    FeedforwardANN step(source);
    ...
    // std::cout << '\n' << 0 << "'s set\n";
    // std::cout << "Cost:" << cost << '\n';
    // std::cout << "Accuracy: " << 100 - (cost / (size_for_fitness * source.output_size())) * 100 << '\n';
    ...
    while(true) {
        ...
        for(int k = 0; k < size_for_fitness; k++) {
            ...
            //current.print();
            //std::cout << "---------------------------------------------------- \n";
            //current.gradient(data).print();
            step += current.gradient(data);
        }
        //step.print();
        step *= ((double)-1 / size_for_fitness) * 0.005;
        //step *= (1 / size_for_fitness) * 1000;
        current += step;
